...
export class SequencesClient {
  ...
  public initialize(): boolean {
    ...
    try {
      ...
      this.enrollmentsTable = this.base(
        process.env.AIRTABLE_ENROLLMENTS_TABLE || 'SequenceEnrollments'
      );
      this.emailTemplatesTable = this.base(
        process.env.AIRTABLE_EMAIL_TEMPLATES_TABLE || 'EmailTemplates'
      );
      this.linkedinTemplatesTable = this.base(
        process.env.AIRTABLE_LINKEDIN_TEMPLATES_TABLE || 'LinkedInTemplates'
      );
  ...
  private recordToSequence(record: AirtableRecord<FieldSet>, steps: SequenceStep[] = []): Sequence {
    ...
    return {
      ...
      schedule: fields.Schedule ? JSON.parse(fields.Schedule as string) : createDefaultSchedule(),
      ...
      metrics: fields.Metrics ? JSON.parse(fields.Metrics as string) : createEmptySequenceMetrics(),
  ...
  private recordToStep(record: AirtableRecord<FieldSet>): SequenceStep {
    ...
    return {
      ...
      linkedinConfig: fields.LinkedInConfig
        ? JSON.parse(fields.LinkedInConfig as string)
        : undefined,
      ...
      conditionConfig: fields.ConditionConfig
        ? JSON.parse(fields.ConditionConfig as string)
        : undefined,
  ...
  private recordToEnrollment(record: AirtableRecord<FieldSet>): SequenceEnrollment {
    ...
    return {
      ...
      completedSteps: fields.CompletedSteps
        ? (fields.CompletedSteps as string).split(',').filter(Boolean)
        : [],
    ...
    try {
      ...
      for (const stepInput of input.steps) {
        const stepRecord = await this.stepsTable!.create({
          ...
          LinkedInConfig: stepInput.linkedinConfig
            ? JSON.stringify(stepInput.linkedinConfig)
            : undefined,
          ...
          ConditionConfig: stepInput.conditionConfig
            ? JSON.stringify(stepInput.conditionConfig)
            : undefined,
   ...
   * Update a step
   */
  public async updateStep(stepId: string, update: Partial<SequenceStep>): Promise<SequenceStep> {
    if (!this.isInitialized()) {
      throw new Error('Sequences client not initialized');
    }
    ...
    if (update.linkedinConfig !== undefined)
      fields.LinkedInConfig = JSON.stringify(update.linkedinConfig);
  ...
  public async advanceEnrollment(
    ...
    try {
      ...
      console.error('Error fetching enrollment:', error);
      throw new Error(
        `Enrollment not found: ${enrollmentId}. ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to advance enrollment: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
